...
function(dat, output_name){
          ...
          if ("Category" %in% colnames(dat)){
                    ...
                    dat_plot <-   dat %>% gather(key="Sample", value="fpkm",-Gene, -Category) %>% mutate(Sample=as_factor(Sample), Gene=as_factor(Gene))
                    
                    #--- Plot heatmap
                   gg= ggplot(data = dat_plot, aes(x = Sample, y = Gene)) +
                              geom_tile(aes(fill = log2(fpkm)), colour = "white") + 
                              facet_wrap(~Category, nrow=3,scales = "free_y", strip.position="top") +
                   ...
                   print(gg)
                   ggsave(filename = paste(output_name,"_heatmap_in_ggplot.png"), device = "png", path="./", width = 8, height =9,unit="cm", dpi=300)
                    
          }
          
          else{
                    message("no category provided! plotting data...")
                    dat_plot <-   dat %>% gather(key="Sample", value="fpkm",-Gene) %>% mutate(Sample=as_factor(Sample), Gene=as_factor(Gene))
